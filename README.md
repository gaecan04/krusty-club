...
**Adding nodes**: the nodes in the path_trace (including the initiator and intermediate nodes) are added to the network_graph if they are not already present. A node_id_to_index map is maintained to correlate the node IDs with the graph indexes.
