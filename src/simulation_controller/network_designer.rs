...
pub(crate) struct NetworkRenderer {
    ...
    pub(crate) node_id_to_index: HashMap<NodeId, usize>,
    config: Option<Arc<Mutex<ParsedConfig>>>,
    next_position_x: f32,
    next_position_y: f32,
    ...
    // for crash/spawn go-back to grid prob-solving
...
impl NetworkRenderer {
    ...
    //👷👷👷creating, building, updating the Network Renderer👷👷👷
    ...
    fn build_topology_layout(&mut self, topo: &str, config: &ParsedConfig, previous_states: &HashMap<usize, bool>, previous_nodes: &Vec<Node>){
        ...
        let client_y = max_y + 80.0;
        let server_y = min_y - 80.0;
        let spacing = 100.0;
    ...
    pub fn position_new_drone(&mut self, new_drone_id: NodeId, connections: &[NodeId]) -> (f32, f32) {
        if let Some((last_x, last_y)) = self.last_spawned_position {
            let new_x = last_x + 50.0;
            let new_y = last_y + 20.0;
